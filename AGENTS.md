...
## Overview
This app runs saved KQL test searches across eight time windows (T1 1h … T8 14d). It records a warm-up plus configurable measured repetitions per window in the app-scoped KV store, and reports **how long each search took start to finish** per tier, with a cross-tier comparison view and clipboard export.
...
## Architecture
`src/App.tsx` is the workbench; `src/routes/SearchesPage.tsx` manages the test-search library; `src/routes/ComparePage.tsx` is the cross-tier comparison (`src/api/compare.ts` derives it, `src/api/exportResults.ts` serialises it); `src/routes/SettingsPage.tsx` holds config. `src/api/kv.ts` is a multi-key KV accessor — the framework's `loadSettings`/`saveSettings` hardcode a single key, and config, run history and the library have very different write rates. They are therefore three separate KV records (`settings`, `runs`, `searches`). Engine inventory and resize use the Search API scoped to the configured worker group. No backend endpoint or external proxy is required, so there is no `config/backend.yml` and no `backend/` directory.
